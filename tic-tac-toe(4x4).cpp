...
struct Board {
    ...
    Board() {
        cells.resize(BOARD_SIZE, vector<char>(BOARD_SIZE, EMPTY));
    }
    ...
    bool isWinningMove(const Move &move, char player) const {
        int row = move.row;
        int col = move.col;

        // Check row
        if (cells[row][0] == player && cells[row][1] == player &&
            cells[row][2] == player && cells[row][3] == player) {
            return true;
        }

        // Check column
        if (cells[0][col] == player && cells[1][col] == player &&
            cells[2][col] == player && cells[3][col] == player) {
            return true;
        }

        // Check main diagonal (top-left to bottom-right)
        if (row == col) {
            if (cells[0][0] == player && cells[1][1] == player &&
...
int evaluate(const Board &board) {
    ...
    for (int col = 0; col < BOARD_SIZE; ++col) {
        if (board.cells[0][col] == board.cells[1][col] &&
            ...
            board.cells[2][col] == board.cells[3][col]) {
    ...
    if (board.cells[0][0] == board.cells[1][1] &&
        ...
        board.cells[2][2] == board.cells[3][3]) {
    ...
    // Check anti-diagonal
    if (board.cells[0][3] == board.cells[1][2] &&
        board.cells[1][2] == board.cells[2][1] &&
        board.cells[2][1] == board.cells[3][0]) {
        if (board.cells[0][3] == PLAYER_X) {
            return 1;
        } else if (board.cells[0][3] == PLAYER_O) {
            return -1;
        }
    }
...
int minimax(Board &board, int depth, bool isMaximizingPlayer) {
    ...
    if (isMaximizingPlayer) {
        ...
        // Find the maximum score by recursively evaluating each possible move
        ...
        // Find the minimum score by recursively evaluating each possible move
        for (int row = 0; row < BOARD_SIZE; ++row) {
            for (int col = 0; col < BOARD_SIZE; ++col) {
                if (board.cells[row][col] == EMPTY) {
